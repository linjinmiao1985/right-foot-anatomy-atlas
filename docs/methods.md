...
| **Soft-tissue open-data ceiling** | Exhaustive search (digs #1–#178, Day 4cl–4dk Week 2–3) found **no CC0/BY per-toe DI, lumbricals, per-ray MTA** meshes | **Grouped structures** (DI 1st–4th combined, dorsal/plantar MTA all rays) are **teaching compromises**, not per-toe/per-ray elemental atlases (详见 `docs/week2-soft-ceiling-memo.md`) |
...
| **BY-SA share** | ≈**71/124** unique structures live under ShareAlike isolate (`by-sa/` directory) | Exhaustive CC0/BY search (digs #1–#178) found DI / per-ray MTA / gastroc-soleus alternatives dry; soft-tissue open-data ceiling reached; NC (Non-Commercial) sources rejected (Zenodo Scan-the-World, Visible Korean, BoneHub) |
